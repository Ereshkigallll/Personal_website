...
function SystemStatus() {
  // SERVER_LOAD: 20–75%, floats slowly
  const load = useMotionValue(35)
  const loadWidth = useTransform(load, v => `${Math.round(v)}%`)

  // NET_LATENCY: 5–80ms, snappier updates
  const latency = useMotionValue(12)
  const displayLatency = useTransform(latency, v => `${Math.round(v)}ms`)

  useEffect(() => {
    // Server load: drifts every 2.5s with slow transitions
    const loadTick = () => {
      const next = Math.round(20 + Math.random() * 55)
      animate(load, next, { duration: 1.2, ease: 'easeInOut' })
    }
    const loadId = setInterval(loadTick, 2500)

    // Net latency: updates every 1.8s, faster transition
    const latencyTick = () => {
    ...
    const latencyId = setInterval(latencyTick, 1800)
  ...
  // Colour the latency value based on magnitude
  const [latencyVal, setLatencyVal] = [latency, latency]
